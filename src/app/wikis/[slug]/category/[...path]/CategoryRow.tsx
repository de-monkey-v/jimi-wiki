"use client";
import { PageKebabMenu } from "@/components/PageKebabMenu";
import { PreviewLink } from "@/components/ui/PreviewLink";

/**
 * 카테고리 목록 한 행. 데스크톱에서 hover 시 케밥(⋮) 액션이 드러난다 — opacity로만 숨겨
 * 키보드 포커스(focus-within)로도 도달 가능하고, 메뉴가 열린 동안(aria-expanded)은 hover가
 * 풀려도 유지되며, 모바일(md 미만)은 항상 표시.
 */
export type CategoryRowProps = {
  wikiSlug: string;
  pageSlug: string;
  title: string;
  categoryLabel: string | null;
  kindLabel: string;
  movable: boolean;
  canTrash: boolean;
  currentCategory: string | null;
  currentVersion: number;
  navProps?: React.LiHTMLAttributes<HTMLLIElement>; // useListNav 컨테이너가 주입(키보드 탐색 활성 표시)
};

export function CategoryRow({
  wikiSlug,
  pageSlug,
  title,
  categoryLabel,
  kindLabel,
  movable,
  canTrash,
  currentCategory,
  currentVersion,
  navProps,
}: CategoryRowProps) {
  return (
    <li {...navProps} className="group/row row-hover -mx-2 flex flex-wrap items-center gap-2 px-2 py-1 data-active:bg-stone-100">
      <PreviewLink
        pageSlug={pageSlug}
        href={`/wikis/${wikiSlug}/${encodeURIComponent(pageSlug)}`}
        className="ui-link rounded"
      >
        {title}
      </PreviewLink>
      {categoryLabel && <span className="text-xs text-stone-400">{categoryLabel}</span>}
      <span className="text-xs text-stone-300">{kindLabel}</span>
      <PageKebabMenu
        wikiSlug={wikiSlug}
        pageSlug={pageSlug}
        currentVersion={currentVersion}
        currentCategory={currentCategory}
        canMove={movable}
        canTrash={canTrash}
        triggerClassName="-my-1 ml-auto flex h-8 w-8 shrink-0 items-center justify-center rounded text-base text-stone-400 md:opacity-0 md:group-focus-within/row:opacity-100 md:group-hover/row:opacity-100 md:aria-expanded:opacity-100"
      />
    </li>
  );
}
