...
The application-owned runtime root is `~/.local/share/jimi-wiki`. It is not a Git
...
If the `wiki-personal` Hermes profile has a configured `jimi-wiki` MCP server, activation also atomically copies the target release's `skills/wiki-ingest` bundle into that profile, restarts an already-running `hermes-gateway.service`, and verifies that MCP reconnects with the release's required tools. Profile connect/disconnect/sync operations share a profile-local `flock`, so they cannot interleave with this activation step. A failed Hermes verification restores the previous skill bundle and aborts activation; the deployment recovery path then restores the old app release and re-synchronizes Hermes to it. An intentionally stopped gateway is not started merely by activation.
